...
- **S3 Access Point**: write **"S3 Access Points"** in full. **"S3 AP" is not a short form AWS sanctions — do not use it.** Where it could be read as an Amazon S3 access point, write "FSx for ONTAP S3 Access Points". Never "FSx S3 AP". **Existing occurrences are grandfathered — do not bulk-rewrite them.**
